 ...
 * the one `<Planner>` instance considers "home" — the two are siblings, not
 * parent/child, so there's no prop path between them. `Planner` registers
 * its own handler on mount; the header calls it, oblivious to what it does.
 */
interface HomeNav {
  goHome: (() => void) | null;
...
export function HomeNavProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <HomeNavContext.Provider value={{ goHome, setGoHome }}>
      {children}
