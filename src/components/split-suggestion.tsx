...
export function SplitSuggestion({
  ...
  return (
    <AdjustPlanPanel collapsedSummary={goalSummary}>
      ...
      <div className="bg-secondary/40 rounded-lg border p-4 text-center">
        ...
        {diffVsMax !== 0 && (
          <div className="text-muted-foreground mt-1 text-xs">
            ...
            i föräldrapenning mot max — men hushållet behåller mer lön (se ovan)
          </div>
        )}
        {diffVsMax === 0 && (
      ...
      {rec.doubleDays > 0 && (
        <p className="text-foreground text-xs">
          Inkluderar {rec.doubleDays} dubbeldagar då ni är lediga samtidigt — det
          motsvarar {rec.doubleDays * 2} dagar ur den gemensamma potten.
        </p>
      )}
      <p className="text-muted-foreground text-xs">
        De reserverade dagarna (90 per vårdnadshavare) behålls alltid. Beloppet
        är före skatt; eventuell föräldralön visas separat ovan.
      </p>
    </AdjustPlanPanel>
  );
}
